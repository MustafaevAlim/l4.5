...
type PreparedStatements struct {
	insertOrder    *sqlx.NamedStmt
	insertDelivery *sqlx.NamedStmt
	insertPayment  *sqlx.NamedStmt
...
func Init() *Storage {
	...
	dsn := fmt.Sprintf("host=%s user=%s password=%s dbname=%s sslmode=disable",
		conf.Repository.Host, conf.Repository.User, conf.Repository.Password, conf.Repository.RepositoryName)
	...
	return &Storage{db: db, stmts: stmts}
}

func prepareStatements(db *sqlx.DB) (*PreparedStatements, error) {
	insertOrder, err := db.PrepareNamed(
	...
	insertDelivery, err := db.PrepareNamed(
	...
	insertPayment, err := db.PrepareNamed(
	...
	insertItem, err := db.PrepareNamed(
...
func (s *Storage) Close() error {
	s.stmts.insertOrder.Close()
	s.stmts.insertDelivery.Close()
	s.stmts.insertPayment.Close()
	s.stmts.insertItem.Close()
	return s.db.Close()
}
...
func (s *Storage) AddOrder(ctx context.Context, order model.Order) error {
	...
	if err := s.insertDeliveryTx(ctx, tx, &order.Delivery, order.OrderUID); err != nil {
		return err
	}
	if err := s.insertPaymentTx(ctx, tx, &order.Payment, order.OrderUID); err != nil {
		return err
	}
	if err := s.insertItemsTx(ctx, tx, order.Items, order.OrderUID); err != nil {
		return err
	}
...
func (s *Storage) insertOrderTx(ctx context.Context, tx *sqlx.Tx, order *model.Order) error {
	stmt := tx.NamedStmtContext(ctx, s.stmts.insertOrder)
	_, err := stmt.ExecContext(ctx, order)
	if err != nil {
...
func (s *Storage) insertDeliveryTx(ctx context.Context, tx *sqlx.Tx, delivery *model.Delivery, orderUID string) error {
	delivery.OrderUid = orderUID
	stmt := tx.NamedStmtContext(ctx, s.stmts.insertDelivery)
	_, err := stmt.ExecContext(ctx, delivery)
	if err != nil {
...
func (s *Storage) insertPaymentTx(ctx context.Context, tx *sqlx.Tx, payment *model.Payment, orderUID string) error {
	payment.OrderUid = orderUID
	stmt := tx.NamedStmtContext(ctx, s.stmts.insertPayment)
...
func (s *Storage) insertItemsTx(ctx context.Context, tx *sqlx.Tx, items []model.Item, orderUID string) error {
	stmt := tx.NamedStmtContext(ctx, s.stmts.insertItem)

	for i := range items {
		items[i].OrderUid = orderUID
		_, err := stmt.ExecContext(ctx, items[i])
		if err != nil {
			return fmt.Errorf("ошибка при вставке позиции: %w", err)
		}
	}
	return nil
}
